export function draw() {
  ...
  const str = '当内容特别多的时候，canvas不会自动换行，需要特别处理当内容特别多的时候，canvas不会自动换行，需要特别处理当内容特别多的时候，canvas不会自动换行，需要特别处理当内容特别多的时候，canvas不会自动换行，需要特别处理'
  ...
  let initHeight = 25 // 绘制字体距离canvas顶部初始的高度
  let lastSunStrIndex = 0 // 每次开始截取的字符串的索引
  let contentWidth = 0
